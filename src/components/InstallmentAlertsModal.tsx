...
export default function InstallmentAlertsModal({ isOpen, onClose, onPaymentPaid }: InstallmentAlertsModalProps) {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div style={{ background: '#ffffff', borderRadius: '8px', width: '800px', maxWidth: '95%', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #eee', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ margin: 0, fontSize: '20px', fontWeight: 600, color: '#2a3142' }}>Installment Alerts</h3>
        ...
        <div style={{ padding: '16px 24px', borderBottom: '1px solid #eee' }}>
          <span style={{ color: '#9ca8b3', fontSize: '14px' }}>Total: </span>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '0 24px 24px' }}>
          {loading ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#9ca8b3' }}>Loading...</div>
          ) : data?.payments.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '40px', color: '#9ca8b3' }}>
              No {activeTab} payments
            </div>
          ) : (
            <table style={{ width: '100%', fontSize: '14px', marginTop: '16px' }}>
              <thead style={{ background: '#f8f9fa' }}>
                <tr>
                  <th style={{ padding: '10px', textAlign: 'left', fontWeight: 600, color: '#525f80' }}>Customer</th>
                  <th style={{ padding: '10px', textAlign: 'left', fontWeight: 600, color: '#525f80' }}>Car ID</th>
                  <th style={{ padding: '10px', textAlign: 'left', fontWeight: 600, color: '#525f80' }}>Installment</th>
                  <th style={{ padding: '10px', textAlign: 'left', fontWeight: 600, color: '#525f80' }}>Amount</th>
                  <th style={{ padding: '10px', textAlign: 'left', fontWeight: 600, color: '#525f80' }}>Due Date</th>
                  <th style={{ padding: '10px', textAlign: 'left', fontWeight: 600, color: '#525f80' }}>Status</th>
                  <th style={{ padding: '10px', textAlign: 'left', fontWeight: 600, color: '#525f80' }}>Action</th>
                </tr>
              </thead>
              <tbody>
                {data?.payments.map((payment) => (
                  <tr key={`${payment.saleId}-${payment.installmentNumber}`} style={{ borderBottom: '1px solid #f5f5f5' }}>
                    ...
                    <td style={{ padding: '10px', fontWeight: 600 }}>${payment.amount.toLocaleString()}</td>
                    <td style={{ padding: '10px' }}>{new Date(payment.dueDate).toLocaleDateString()}</td>
                    <td style={{ padding: '10px' }}>
                      <span style={{
                        padding: '4px 8px',
                        borderRadius: '4px',
                        ...
                        {activeTab === 'overdue' ? `${payment.daysOverdue} days overdue` : `Due in ${payment.daysUntilDue} days`}
                      </span>
                    </td>
                    <td style={{ padding: '10px' }}>
                      <button
                      ...
                      >
                        {processingId === `${payment.saleId}-${payment.installmentNumber}` ? 'Processing...' : 'Mark Paid'}
                      </button>
                    </td>
                  </tr>
                ))}
