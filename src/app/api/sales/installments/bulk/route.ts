...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'cancel') {
      const sales = await InstallmentSale.find({ _id: { $in: ids }, status: 'Active' });
      ...
      if (activeIds.length > 0) {
        ...
        // Delete associated transactions (purchase and payments)
        ...
        await logActivity({
          userId: auth.userId,
          userName: auth.name,
          action: `Bulk cancelled ${activeIds.length} installment sales`,
          module: 'Sales',
      ...
      return NextResponse.json({ 
        message: `Successfully cancelled ${activeIds.length} installment sales.`,
