...
export async function POST(request: NextRequest) {
  try {
    ...
    // Batch: update car status + create down payment transaction in parallel
    const [customerDoc, carDoc] = await Promise.all([
      Car.findByIdAndUpdate(car, { status: 'Reserved' }),
      Transaction.create({
    ...
    try {
      const zatcaResult = await processZatcaInvoice({
        ...
        lineItems: [{
          name: carDoc ? `${carDoc.brand} ${carDoc.model} (${carId})`.trim() : carId,
          quantity: 1,
