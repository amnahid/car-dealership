...
export async function PUT(
  ...
  try {
    ...
    if (status !== undefined && (status === 'Completed' || status === 'Cancelled')) {
      ...
      // Only soft delete transactions if cancelled. Completed rentals should keep their revenue.
      if (status === 'Cancelled') {
...
export async function PATCH(
  ...
  try {
    ...
    if (action === 'revert-cancellation') {
      ...
      if (rental.status !== 'Cancelled') {
        return NextResponse.json({ error: 'Rental is not cancelled' }, { status: 400 });
      }

      const now = new Date();
      const timestamp = now.toLocaleDateString();
      const revertNote = `\n[Reverted on ${timestamp}]: ${reason}`;
      rental.notes = (rental.notes || '') + revertNote;
      ...
      await logActivity({
        userId: user.userId,
        userName: user.name,
        action: `Reverted cancellation for rental: ${rental.rentalId}. Reason: ${reason}`,
        module: 'Sales',
        targetId: rental._id.toString(),
        ipAddress: request.headers.get('x-forwarded-for') || 'unknown',
      });
