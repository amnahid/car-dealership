...
export async function PUT(
  ...
  try {
    ...
    if (action === 'Approve') {
      ...
      // SIDE EFFECTS: If cancellation is approved, update Car and Transactions
      if (
        ['CashSale', 'InstallmentSale', 'Rental'].includes(editReq.targetModel) &&
        editReq.proposedChanges.status === 'Cancelled'
      ) {
        const carId = updatedDoc.car;
        if (carId) {
        ...
        await Transaction.updateMany(
          { referenceId: editReq.targetId.toString(), referenceType: editReq.targetModel, isAutoGenerated: true },
          { isDeleted: true }
        );
      }
