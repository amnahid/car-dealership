...
interface StatsData {
  ...
  carsRented: number;
  carsReserved: number;
  carsOnInstallment: number;
  carsDefaulted: number;
...
function StatCard({
  ...
  const card = (
    <div style={cardStyle} className="stat-card">
      ...
      <p style={{ fontSize: '20px', fontWeight: 700, marginTop: '8px', position: 'relative', wordBreak: 'break-word' }}>
    ...
    <>
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '12px' }}>
          ...
          />
          <StatCard 
            label={t('onInstallment')} 
            value={stats.carsOnInstallment} 
            colorKey="warning" 
            href="/dashboard/cars?status=On+Installment"
            icon={<svg width="110" height="110" viewBox="0 0 24 24" fill="currentColor"><path d="M11.8 10.9c-2.27-.59-3-1.2-3-2.15 0-1.09 1.01-1.85 2.7-1.85 1.78 0 2.44.85 2.5 2.1h2.21c-.07-1.72-1.12-3.3-3.21-3.81V3h-3v2.16c-1.94.42-3.5 1.68-3.5 3.61 0 2.31 1.91 3.46 4.7 4.13 2.5.6 3 1.48 3 2.41 0 .69-.49 1.79-2.7 1.79-2.06 0-2.87-.92-2.98-2.1h-2.2c.12 2.19 1.76 3.35 3.98 3.93V21h3v-2.15c1.95-.37 3.5-1.5 3.5-3.55 0-2.84-2.43-3.81-4.7-4.4z"/><path d="M7 7h2v2H7zm0 4h2v2H7zm0 4h2v2H7z" opacity="0.3"/></svg>}
          />
          <StatCard 
            label={t('reserved')} 
          ...
          <StatCard 
            label={t('defaulted')} 
            value={stats.carsDefaulted} 
            colorKey="danger" 
            href="/dashboard/cars?status=Defaulted"
            icon={<svg width="110" height="110" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm1 15h-2v-2h2v2zm0-4h-2V7h2v6z"/></svg>}
          />
          <StatCard 
