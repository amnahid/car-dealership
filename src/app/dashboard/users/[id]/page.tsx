...
export default function UserDetailPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/users/${id}`)
      ...
      .then((data) => {
        setUser(data.user);
        setEditForm({ name: data.user.name, role: data.user.role });
      })
      .catch((err) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setUser({ ...user!, name: editForm.name, role: editForm.role });
      setEditMode(false);
    } catch (err) {
      console.error(err);
    } finally {
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px' }}>
        <div className="card" style={{ padding: '24px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '24px' }}>
            <h2 style={{ fontSize: '20px', fontWeight: 600, color: '#2a3142', margin: 0 }}>User Details</h2>
            <span style={{
              padding: '4px 12px',
              borderRadius: '12px',
              fontSize: '12px',
          ...
          {!editMode ? (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                <p style={{ fontSize: '12px', color: '#9ca8b3', margin: '0 0 4px' }}>Name</p>
                <p style={{ fontSize: '16px', color: '#2a3142', margin: 0, fontWeight: 500 }}>{user.name}</p>
              </div>
              <div>
                <p style={{ fontSize: '12px', color: '#9ca8b3', margin: '0 0 4px' }}>Email</p>
              ...
              <div>
                <p style={{ fontSize: '12px', color: '#9ca8b3', margin: '0 0 4px' }}>Status</p>
                <p style={{ fontSize: '16px', margin: 0, color: user.isActive ? '#16a34a' : '#ec4561', fontWeight: 600 }}>
                  {user.isActive ? 'Active' : 'Inactive'}
                </p>
              </div>
              <div>
                <p style={{ fontSize: '12px', color: '#9ca8b3', margin: '0 0 4px' }}>Created</p>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                <label style={{ fontSize: '12px', color: '#9ca8b3', display: 'block', marginBottom: '4px' }}>Name</label>
                <input
                  type="text"
                  value={editForm.name}
              ...
              <div>
                <label style={{ fontSize: '12px', color: '#9ca8b3', display: 'block', marginBottom: '4px' }}>Role</label>
                <select
                  value={editForm.role}
              ...
              <div style={{ display: 'flex', gap: '8px' }}>
                ...
                <button onClick={() => { setEditMode(false); setEditForm({ name: user.name, role: user.role }); }} style={{ padding: '10px 20px', background: '#fff', color: '#6b7280', border: '1px solid #e5e5e5', borderRadius: '4px', cursor: 'pointer' }}>
                  Cancel
                </button>
              </div>
            </div>
