  ...
  const handleExportExcel = () => {
    ...
    const workbook = `<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet">
      <Worksheet ss:Name="Expenses"><Table>
        ...
        ${expenses.map(e => `<Row><Cell><Data ss:Type="String">${e.date}</Data></Cell><Cell><Data ss:Type="String">${e.category}</Data></Cell><Cell><Data ss:Type="String">${e.description.replace(/"/g, '&quot;')}</Data></Cell><Cell><Data ss:Type="Number">${e.amount}</Data></Cell></Row>`).join('')}
    ...
    <div style={{ marginBottom: '24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '16px', marginBottom: '24px' }}>
        <div className="card" style={{ padding: '20px', borderLeft: '4px solid #ec4561' }}>
          ...
          <p style={{ fontSize: '28px', fontWeight: 700, color: '#ec4561', margin: '4px 0 0' }}>SAR {summary.total.toLocaleString()}</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px', marginBottom: '24px' }}>
        <div className="card" style={{ padding: '20px' }}>
          <h4 style={{ marginTop: 0, marginBottom: '16px', color: '#525f80' }}>Expenses by Category</h4>
        ...
        <div className="card" style={{ padding: '20px' }}>
          <h4 style={{ marginTop: 0, marginBottom: '16px', color: '#525f80' }}>Monthly Expenses</h4>
          <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
        {loading ? (
          ...
          <div style={{ padding: '32px', textAlign: 'center', color: '#9ca8b3' }}>No expenses found.</div>
        ) : (
              ...
              <tbody style={{ borderBottom: '1px solid #eee' }}>
                {expenses.map(exp => (
                  <tr key={exp._id} style={{ borderBottom: '1px solid #f5f5f5' }}>
                    <td style={{ padding: '12px', color: '#525f80' }}>{new Date(exp.date).toLocaleDateString()}</td>
                    <td style={{ padding: '12px' }}>{exp.category}</td>
                    <td style={{ padding: '12px' }}>{exp.description}</td>
                    <td style={{ padding: '12px', fontWeight: 600, color: '#ec4561' }}>SAR {exp.amount.toLocaleString()}</td>
                  </tr>
                ))}
              </tbody>
            </table>
